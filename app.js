...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    nB.onclick=async()=>{
        ...
        try{
            ...
            let raw=d.choices[0].message.content.replace(/^\s*```json/i,'').replace(/```\s*$/i,'');
            let fJ;
            try{ fJ=JSON.parse(raw); }
            catch(pe){ throw new Error('AI returned invalid JSON. Please try again. Details: '+pe.message); }
